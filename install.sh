...
if command -v apt-get >/dev/null 2>&1; then export DEBIAN_FRONTEND=noninteractive;apt-get update -qq;apt-get install -y -qq curl ca-certificates python3 tar coreutils
elif command -v dnf >/dev/null 2>&1; then dnf -y -q install curl ca-certificates python3 tar coreutils
elif command -v yum >/dev/null 2>&1; then yum -y -q install curl ca-certificates python3 tar coreutils
...
META="$WORK/release.json"; step 'Fetching latest release of BUB-Panel'; curl "${CURL[@]}" -sS --max-time 30 -o "$META" "$API" || die 'Could not fetch release metadata.'
...
step 'Verifying SHA256 checksums';(cd "$WORK";grep -E "  (${RUNTIME}|${PANEL}|${AGENT})$" "$SUMS"|sha256sum -c -)||die 'Checksum verification failed.';ok 'release assets verified'
